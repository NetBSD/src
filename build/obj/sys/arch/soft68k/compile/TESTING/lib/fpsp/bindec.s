...
iscale:
	...
	addw	%d1,%d1		|put them in bits 2:1
	addw	%d5,%d1		|add in LAMBDA
	addw	%d1,%d1		|put them in bits 3:1
...
x_pos:
	...
	lea	PTENRN,%a1		|load %a1 with RN table base
	bras	rmode		|exit decode
not_rn:
	lsrb	#1,%d3		|get lsb in carry
	bccs	not_rp		|if carry clear, it is RM
	lea	PTENRP,%a1		|load %a1 with RP table base
	bras	rmode		|exit decode
not_rp:
	lea	PTENRM,%a1		|load %a1 with RM table base
rmode:
...
A9_str:
	...
	bnes	short_sc_mul		|if neg (LAMBDA = 1), scale by mul
