...
L2:
	.long	0x3FDC0000,0x82E30865,0x4361C4C6,0x00000000

EXPA3:
	.long	0x3FA55555,0x55554431
EXPA2:
	.long	0x3FC55555,0x55554018
...
EM1A4:
	.long	0x3F811111,0x11174385
EM1A3:
	.long	0x3FA55555,0x55554F5A

EM1A2:
	.long	0x3FC55555,0x55555555,0x00000000,0x00000000

EM1B8:
	.long	0x3EC71DE3,0xA5774682
EM1B7:
	.long	0x3EFA01A0,0x19D7CB68

EM1B6:
	.long	0x3F2A01A0,0x1A019DF3
EM1B5:
	.long	0x3F56C16C,0x16C170E2

EM1B4:
	.long	0x3F811111,0x11111111
EM1B3:
	.long	0x3FA55555,0x55555555

EM1B2:
	.long	0x3FFC0000,0xAAAAAAAA,0xAAAAAAAB
	.long	0x00000000

TWO140:
	.long	0x48B00000,0x00000000
TWON140:
...
|--This is the normal branch:	2^(-65) <= |X| < 16380 log2.
	...
	fmovex	%fp0,%fp1
	fmuls	#0r92.332481384277343750,%fp0		|...64/log2 * X
...
|--%a0 points to 2^(J/64), %D0 is biased expo. of 2^(M)
	fmovex	%fp0,%fp2
	fmuls	#0r-0.0108304247260093688964843750,%fp0		|...N * L1, L1 = lead(-log2/64)
...
|--[R+R*S*(%A2+S*%A4)] + [S*(%A1+S*(%A3+S*%A5))]
	...
	fmoves	#0r0.001388890668749809265136718750,%fp2		|...%fp2 IS %A5
|	CLR.W		%a1@(2)		...load 2^(J/64) in cache

	fmulx	%fp1,%fp2		|...%fp2 IS S*%A5
	fmovex	%fp1,%fp3
	fmuls	#0r0.0083333449438214302062988281250,%fp3		|...%fp3 IS S*%A4
	...
	fadds	#0r0.5,%fp2		|...%fp2 IS %A1+S*(%A3+S*%A5)
...
|--Step 7
	...
	fmovel	%d1,%FPCR
	fadds	#0r1.0,%fp0		|...1+X in user mode
	bra	t_frcinx
...
|--[R*S*(%A2+S*(%A4+S*%A6))] + [R+S*(%A1+S*(%A3+S*%A5))]
	...
	fmoves	#0r0.0001983995753107592463493347167968750,%fp2		|...%fp2 IS %a6
|	CLR.W		%a1@(2)		...load 2^(J/64) in cache

	fmulx	%fp1,%fp2		|...%fp2 IS S*%A6
	fmovex	%fp1,%fp3
	fmuls	#0r0.001388889970257878303527832031250,%fp3		|...%fp3 IS S*%A5
	...
	fadds	#0r0.5,%fp3		|...%fp3 IS %A1+S*(%A3+S*%A5)
...
|--Step 9	exp(X)-1 by a simple polynomial
	...
	fmovemx	%fp2/%fp3,%a7@-		|...save %fp2
	fmoves	#0r1.60791047143504783889511600136756896972656250e-10,%fp1		|...%fp1 is B12
	fmulx	%fp0,%fp1		|...%fp1 is S*B12
	fmoves	#0r2.0883454965314740547910332679748535156250e-09,%fp2		|...%fp2 is B11
	fadds	#0r2.5052088403754169121384620666503906250e-08,%fp1		|...%fp1 is B10+S*B12

	fmulx	%fp0,%fp2		|...%fp2 is S*B11
	fmulx	%fp0,%fp1		|...%fp1 is S*(B10 + ...

	fadds	#0r2.755732850800995947793126106262207031250e-07,%fp2		|...%fp2 is B9+S*...
...
|--Step 10	|X| > 70 log2
	...
	bgt	EXPC1
|--Step 10.2
	fmoves	#0r-1.0,%fp0		|...%fp0 is -1
	fmovel	%d1,%FPCR
		.long	0xf23c4422,0x00800000		|...-1 + 2^(-126)
