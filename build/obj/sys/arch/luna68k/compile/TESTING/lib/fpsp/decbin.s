...
|	word to 0x0100.  This will signal unimp.sa that an enabled inex1
...
m_sign:
	...
	beqs	short_ap_st_z		|if clear, go to append/strip zeros
...
short_ap_st_z:
	...
	bnes	short_ap_st_n		|if neg, go to neg side
...
ap_p_en:
	...
	bras	short_pwrten		|go calc pwrten
|
...
short_pwrten:
	...
	bccs	not_rp		|to %a1
	lea	PTENRP,%a1		|it is RP
	bras	calc_p		|go to init section
not_rp:
	asrl	#1,%d0		|keep checking
	bccs	not_rm
	lea	PTENRM,%a1		|it is RM
	bras	calc_p		|go to init section
not_rm:
	lea	PTENRN,%a1		|it is RN
calc_p:
