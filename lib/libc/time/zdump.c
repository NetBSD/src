/*	$NetBSD: zdump.c,v 1.63.2.1 2025/08/02 05:54:44 perseant Exp $	*/
...
#include <sys/cdefs.h>
#ifndef lint
__RCSID("$NetBSD: zdump.c,v 1.63.2.1 2025/08/02 05:54:44 perseant Exp $");
#endif /* !defined lint */
...
gmtzinit(void)
{
  if (USE_LOCALTIME_RZ) {
    ...
    if (!gmtz) {
      ...
      gmtz = tzalloc(gmt0);
      if (!gmtz) {
	err(EXIT_FAILURE, "Cannot create %s", gmt0);
      }
    }
  }
}
