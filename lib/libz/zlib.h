...
ZEXTERN int ZEXPORT inflateIncomp __P((z_streamp));
...
ZEXTERN int ZEXPORT compress2 __P((Bytef *, uLongf *, const Bytef *, uLong,
    int));
/*
...
ZEXTERN long ZEXPORT    gzseek __P((gzFile, long, int));
ZEXTERN z_off_t ZEXPORT    __gzseek1 __P((gzFile, z_off_t, int));
#else
 ...
 */
#ifdef __ZLIB_BACKWARD_COMPAT
ZEXTERN long ZEXPORT    gztell __P((gzFile));
ZEXTERN z_off_t ZEXPORT    __gztell1 __P((gzFile));
#else
ZEXTERN z_off_t ZEXPORT    gztell __P((gzFile)) __RENAME(__gztell1);
#endif
/*
...
ZEXTERN int ZEXPORT deflateInit2_ __P((z_streamp, int, int, int, int, int,
    const char *, int));
