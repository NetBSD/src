...
# Show tags only matching certain regular expression.
my $Regexp_Tag = '';
...
sub header_line {
  my $self = shift;
  my ($time, $author, $lastdate, $lastauthor) = @_;
  ...
  if ($Show_Times) {
    ...
    if ( $GroupByDate and ($date eq $lastdate) and
         ((!$GroupByAuthor) or ($author eq $lastauthor)) ) {
      $header_line = '';
    } else {
      if ( $GroupByDate and ! $GroupByAuthor ) {
        $header_line = "$date\n\n";
      } else {
      ...
      foreach my $time (@key_list) {
        ...
        while (my ($author,$mesghash) = each %$authorhash) {
          while (my ($msg,$qunk) = each %$mesghash) {
            ...
            for my $qunkref2 (@$qunklist) {
              if (!$self->_revision_is_wanted($qunkref2)) {
                if (defined ($qunkref2->tags)) {
                  for my $tag (@{$qunkref2->tags}) {
                    $tag_date_printed{$tag} = 1;
                  }
                }
              }
            }
    ...
    foreach my $time (@key_list)
    {
      ...
      if ( $Show_Tag_Dates || $XML_Output ) {
        ...
        foreach my $tag (sort keys %tags) {
          if ( ! defined $tag_date_printed{$tag} ) {
            ...
            if ( (! defined $Regexp_Tag) or ( $tag =~ /$Regexp_Tag/ ) ) {
      ...
      FOOBIE:
        ...
        for my $msg (sort keys %$mesghash)
        {
          ...
          my $date = $self->fdatetime($time);
          $header_line = $self->header_line($time, $author, $lastdate, $lastauthor);
          $lastdate = $date;
          $lastauthor = $author;
...
=item B<--group-by-date>

Group ChangeLog entries on the same date together, instead of having a
separate entry for each commit on that date.

=item B<--group-by-author>

Group consecutive ChangeLog entries from same author during same date,
instead of having separate entry for each commit.
