 ...
 */
typedef enum {
	OPS_PTAG_OF_LT_ONE_BYTE = 0x00,	/* !< Packet has a 1 byte length -
					 * header is 2 bytes long. */
	OPS_PTAG_OF_LT_TWO_BYTE = 0x01,	/* !< Packet has a 2 byte length -
					 * header is 3 bytes long. */
	OPS_PTAG_OF_LT_FOUR_BYTE = 0x02,	/* !< Packet has a 4 byte
						 * length - header is 5 bytes
						 * long. */
	OPS_PTAG_OF_LT_INDETERMINATE = 0x03	/* !< Packet has a
...
typedef struct {
	...
	unsigned        content_tag;	/* !< content_tag value - See
					 * #ops_content_tag_t for meanings */
	ops_ptag_of_lt_t length_type;	/* !< Length type (#ops_ptag_of_lt_t)
...
typedef struct {
	...
	unsigned        days_valid;	/* !< validity period of the key in
					 ...
					 * used with v3 keys. */
	ops_public_key_algorithm_t algorithm;	/* !< Public Key Algorithm
						 * type */
...
typedef struct {
	...
	unsigned char   signer_id[OPS_KEY_ID_SIZE];	/* !< Eight-octet key ID
							 * of signer */
	ops_public_key_algorithm_t key_algorithm;	/* !< public key
							 * algorithm number */
	ops_hash_algorithm_t hash_algorithm;	/* !< hashing algorithm
						 * number */
...
typedef struct {
	...
	size_t          v4_hashed_data_start;	/* only valid if accumulate
						 * is set */
	ops_hash_t     *hash;	/* !< if set, the hash filled in for the data
				 * so far */
}               ops_signature_t;
