...
struct ops_reader_info {
	...
	ops_parse_info_t *pinfo;/* !< A pointer back to the parent parse_info
				 * structure */
};
...
struct ops_crypt_info {
	...
	ops_parse_cb_t *cb_get_passphrase;	/* <! callback to use to get
						 * the passphrase */
};

/** ops_parse_cb_info */
struct ops_parse_cb_info {
	ops_parse_cb_t *cb;	/* !< the callback function to use when
				 * parsing */
