...
void 
ops_fast_create_user_id(ops_user_id_t * id, unsigned char *user_id)
{
	id->user_id = user_id;
}
...
static unsigned 
public_key_length(const ops_public_key_t * key)
{
...
static unsigned 
secret_key_length(const ops_secret_key_t * key)
{
	int             l;
...
*/
void 
ops_fast_create_rsa_public_key(ops_public_key_t * key, time_t t,
			       BIGNUM * n, BIGNUM * e)
{
	key->version = 4;
	key->creation_time = t;
...
void 
ops_fast_create_rsa_secret_key(ops_secret_key_t * key, time_t t,
 ...
 */
void 
ops_create_info_delete(ops_create_info_t * info)
{
...
ops_create_pk_session_key(const ops_keydata_t * key)
{
	/*
...
ops_memory_t   *
ops_write_mem_from_file(const char *filename, int *errnum)
{
	size_t          initial_size = 1024;
	int             fd = 0;
	unsigned char   buf[1024];
		       ...
		       ops_create_info_t * info)
{
	...
	ops_keyid(keyid, OPS_KEY_ID_SIZE, OPS_KEY_ID_SIZE, &skey->public_key);
