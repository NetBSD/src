...
print_bn(const char *name,
	 const BIGNUM * bn);
static void 
print_hex(const unsigned char *src,
	  size_t length);
static void 
...
static void 
print_string_and_value(const char *name,
		       const char *str,
		       unsigned char value);
static void     print_tagname(const char *str);
static void 
print_time(const char *name,
	   time_t t);
static void     print_time_short(time_t t);
static void 
print_unsigned_int(const char *name,
		   unsigned int val);
...
ops_print_public_keydata(const ops_keydata_t * key)
{
	...
	printf("pub %s ", ops_show_pka(key->key.pkey.algorithm));
...
static void 
print_data(const char *name, const ops_data_t * data)
{
...
static void 
print_packet_hex(const ops_packet_t * packet)
{
	unsigned char  *cur;
	int             i;
	int             rem;
...
static void 
print_text_breakdown(ops_text_t * text)
{
	unsigned        i;
	const char     *prefix = ".. ";
...
static void 
print_headers(const ops_headers_t * headers)
{
	unsigned        n;
...
*/
int 
ops_print_packet(const ops_parser_content_t * content_)
{
...
cb_list_packets(const ops_parser_content_t * content_, ops_parse_cb_info_t * cbinfo)
{
	OPS_USED(cbinfo);
