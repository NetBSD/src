...
*/
void 
ops_writer_push_stream_encrypt_se_ip(ops_create_info_t * cinfo,
				     const ops_keydata_t * pub_key)
{
	ops_crypt_t    *encrypted;
	unsigned char  *iv = NULL;

	const unsigned int bufsz = 1024;	/* initial value; gets
...
static unsigned int 
ops_calc_partial_data_length(unsigned int len)
{
	int             i;
...
static void 
stream_encrypt_se_ip_destroyer(ops_writer_info_t * winfo)
{
	stream_encrypt_se_ip_arg_t *arg = ops_writer_get_arg(winfo);
