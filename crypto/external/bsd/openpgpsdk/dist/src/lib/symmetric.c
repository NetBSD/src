...
static void 
std_set_iv(ops_crypt_t * crypt, const unsigned char *iv)
{
...
static void 
std_set_key(ops_crypt_t * crypt, const unsigned char *key)
{
	memcpy(crypt->key, key, crypt->keysize);
}

static void 
std_resync(ops_crypt_t * decrypt)
{
...
static void 
cast5_block_encrypt(ops_crypt_t * crypt, void *out, const void *in)
{
	CAST_ecb_encrypt(in, out, crypt->encrypt_key, CAST_ENCRYPT);
}

static void 
cast5_block_decrypt(ops_crypt_t * crypt, void *out, const void *in)
{
	CAST_ecb_encrypt(in, out, crypt->encrypt_key, CAST_DECRYPT);
}

static void 
cast5_cfb_encrypt(ops_crypt_t * crypt, void *out, const void *in, size_t count)
{
...
static void 
cast5_cfb_decrypt(ops_crypt_t * crypt, void *out, const void *in, size_t count)
{
	CAST_cfb64_encrypt(in, out, count,
			   crypt->encrypt_key, crypt->iv, &crypt->num,
			   CAST_DECRYPT);
...
static void 
idea_block_encrypt(ops_crypt_t * crypt, void *out, const void *in)
{
	idea_ecb_encrypt(in, out, crypt->encrypt_key);
}

static void 
idea_block_decrypt(ops_crypt_t * crypt, void *out, const void *in)
{
	idea_ecb_encrypt(in, out, crypt->decrypt_key);
}

static void 
idea_cfb_encrypt(ops_crypt_t * crypt, void *out, const void *in, size_t count)
{
...
static void 
idea_cfb_decrypt(ops_crypt_t * crypt, void *out, const void *in, size_t count)
{
	idea_cfb64_encrypt(in, out, count,
...
static void 
aes_block_encrypt(ops_crypt_t * crypt, void *out, const void *in)
{
	AES_encrypt(in, out, crypt->encrypt_key);
}

static void 
aes_block_decrypt(ops_crypt_t * crypt, void *out, const void *in)
{
	AES_decrypt(in, out, crypt->decrypt_key);
}

static void 
aes_cfb_encrypt(ops_crypt_t * crypt, void *out, const void *in, size_t count)
{
...
static void 
aes_cfb_decrypt(ops_crypt_t * crypt, void *out, const void *in, size_t count)
{
	AES_cfb128_encrypt(in, out, count,
			   crypt->encrypt_key, crypt->iv, &crypt->num,
			   AES_DECRYPT);
...
static void 
tripledes_init(ops_crypt_t * crypt)
{
...
static void 
tripledes_block_encrypt(ops_crypt_t * crypt, void *out,
			const void *in)
{
...
static void 
tripledes_block_decrypt(ops_crypt_t * crypt, void *out,
			const void *in)
{
	DES_key_schedule *keys = crypt->encrypt_key;
...
static void 
tripledes_cfb_encrypt(ops_crypt_t * crypt ATTRIBUTE_UNUSED, void *out ATTRIBUTE_UNUSED, const void *in ATTRIBUTE_UNUSED, size_t count ATTRIBUTE_UNUSED)
{
	DES_key_schedule *keys = crypt->encrypt_key;
...
static void 
tripledes_cfb_decrypt(ops_crypt_t * crypt ATTRIBUTE_UNUSED, void *out ATTRIBUTE_UNUSED, const void *in ATTRIBUTE_UNUSED, size_t count ATTRIBUTE_UNUSED)
{
	DES_key_schedule *keys = crypt->encrypt_key;
	DES_ede3_cfb64_encrypt(in, out, count,
	&keys[0], &keys[1], &keys[2], (DES_cblock *) crypt->iv, &crypt->num,
...
static const ops_crypt_t *
get_proto(ops_symmetric_algorithm_t alg)
{
	switch (alg) {
	case OPS_SA_CAST5:
		return &cast5;
...
int 
ops_crypt_any(ops_crypt_t * crypt, ops_symmetric_algorithm_t alg)
{
...
unsigned 
ops_block_size(ops_symmetric_algorithm_t alg)
{
...
unsigned 
ops_key_size(ops_symmetric_algorithm_t alg)
{
	const ops_crypt_t *p = get_proto(alg);
...
void 
ops_encrypt_init(ops_crypt_t * encrypt)
{
...
void 
ops_decrypt_init(ops_crypt_t * decrypt)
{
...
size_t 
ops_encrypt_se(ops_crypt_t * encrypt, void *out_, const void *in_,
	       size_t count)
{
	unsigned char  *out = out_;
	const unsigned char *in = in_;
...
size_t 
ops_encrypt_se_ip(ops_crypt_t * crypt, void *out_, const void *in_,
		  size_t count)
{
...
size_t 
ops_decrypt_se_ip(ops_crypt_t * crypt, void *out_, const void *in_,
		  size_t count)
{
	if (!ops_is_sa_supported(crypt->algorithm))
		return -1;
