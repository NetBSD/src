...
static int 
se_ip_data_reader(void *dest_, size_t len, ops_error_t ** errors,
...
static void 
se_ip_data_destroyer(ops_reader_info_t * rinfo)
{
	decrypt_se_ip_arg_t *arg = ops_reader_get_arg(rinfo);
	free(arg->plaintext);
	free(arg);
 ...
 */
void 
ops_reader_pop_se_ip_data(ops_parse_info_t * pinfo)
{
	/*
	 * decrypt_se_ip_arg_t
	 * *arg=ops_reader_get_arg(ops_parse_get_rinfo(pinfo));
	 */
	/* free(arg); */
