/*	$NetBSD: ssh-pkcs11.c,v 1.24.2.2 2023/11/02 22:15:22 sborrill Exp $	*/
 ...
 */
#include "includes.h"
__RCSID("$NetBSD: ssh-pkcs11.c,v 1.24.2.2 2023/11/02 22:15:22 sborrill Exp $");
...
pkcs11_rsa_start_wrapper(void)
{
	...
	if (rsa_method == NULL)
		return (-1);
	rsa_idx = RSA_get_ex_new_index(0, __UNCONST("ssh-pkcs11-rsa"),
    ...
    const BIGNUM *rp, EC_KEY *ec)
{
	...
	/* XXX handle CKR_BUFFER_TOO_SMALL */
	rv = f->C_Sign(si->session, __UNCONST(dgst), dgst_len, sig, &siglen);
	if (rv != CKR_OK) {
		error("C_Sign failed: %lu", rv);
		goto done;
	}
...
pkcs11_ecdsa_start_wrapper(void)
{
	...
	if (ec_key_method != NULL)
		return (0);
	ec_key_idx = EC_KEY_get_ex_new_index(0, __UNCONST("ssh-pkcs11-ecdsa"),
	    NULL, NULL, pkcs11_k11_free);
	if (ec_key_idx == -1)
		return (-1);
