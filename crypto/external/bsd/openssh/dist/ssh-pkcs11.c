/*	$NetBSD: ssh-pkcs11.c,v 1.29.2.1 2026/02/02 18:08:01 martin Exp $	*/
 ...
 */
#include "includes.h"
__RCSID("$NetBSD: ssh-pkcs11.c,v 1.29.2.1 2026/02/02 18:08:01 martin Exp $");
    ...
    const char *sk_pin, u_int compat)
{
	...
	/* XXX handle CKR_BUFFER_TOO_SMALL */
	rv = f->C_Sign(si->session, __UNCONST(dgst), dgst_len, sig, &slen);
	if (rv != CKR_OK) {
		error("C_Sign failed: %lu", rv);
		goto done;
	}
