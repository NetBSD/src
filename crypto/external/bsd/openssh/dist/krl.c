/*	$NetBSD: krl.c,v 1.21.2.2 2023/11/02 22:15:21 sborrill Exp $	*/
...
#include "includes.h"
__RCSID("$NetBSD: krl.c,v 1.21.2.2 2023/11/02 22:15:21 sborrill Exp $");
...
insert_serial_range(struct revoked_serial_tree *rt, u_int64_t lo, u_int64_t hi)
{
	...
	KRL_DBG(("insert %"PRIu64":%"PRIu64, lo, hi));
	...
	if (ers == NULL || serial_cmp(ers, &rs) != 0) {
		...
		KRL_DBG(("overlap found %"PRIu64":%"PRIu64,
		    ers->lo, ers->hi));
		/*
	...
	while ((crs = RB_PREV(revoked_serial_tree, rt, ers)) != NULL) {
		...
		if (crs->lo < ers->lo) {
			ers->lo = crs->lo;
			KRL_DBG(("pred extend %"PRIu64":%"PRIu64,
			    ers->lo, ers->hi));
		}
	...
	while ((crs = RB_NEXT(revoked_serial_tree, rt, ers)) != NULL) {
		...
		if (crs->hi > ers->hi) {
			ers->hi = crs->hi;
			KRL_DBG(("succ extend %"PRIu64":%"PRIu64,
			    ers->lo, ers->hi));
		}
		RB_REMOVE(revoked_serial_tree, rt, crs);
		free(crs);
	}
	KRL_DBG(("done, final %"PRIu64":%"PRIu64, ers->lo, ers->hi));
	return 0;
}
