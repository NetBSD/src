/*	$NetBSD: sftp.c,v 1.36.2.2 2023/11/02 22:15:22 sborrill Exp $	*/
...
#include "includes.h"
__RCSID("$NetBSD: sftp.c,v 1.36.2.2 2023/11/02 22:15:22 sborrill Exp $");
...
#else
	struct stat ass, bss;
	const struct stat *as = &ass;
	const struct stat *bs = &bss;
#endif
	int rmul = sort_flag & LS_REVERSE_SORT ? -1 : 1;
    ...
    char *file, int remote, int lastarg, char quote, int terminated)
{
	...
	if (remote != LOCAL) {
		...
		(void)glob(tmp, GLOB_LIMIT|GLOB_DOOFFS|GLOB_MARK, NULL, &g);
...
main(int argc, char **argv)
{
	...
	char *host = NULL, *user, *cp, **cpp, *file1 = NULL, *file2 = NULL;
	int debug_level = 0;
