/*	$NetBSD: dnssec.c,v 1.1.1.18 2026/08/29 14:32:12 christos Exp $	*/
...
add_cds(dns_dnsseckey_t *key, dns_rdata_t *keyrdata, const char *keystr,
	...
	if (!dns_rdataset_isassociated(cds) || !exists(cds, &cdsrdata)) {
		...
		isc_log_write(dns_lctx, DNS_LOGCATEGORY_GENERAL,
			      ...
			      keystr);
		return addrdata(&cdsrdata, diff, origin, ttl, mctx);
	}
...
contains_digest(dns_kasp_digestlist_t *digests, unsigned int digesttype) {
	for (dns_kasp_digest_t *alg = ISC_LIST_HEAD(*digests); alg != NULL;
	     alg = ISC_LIST_NEXT(alg, link))
	{
...
delete_cds(dns_dnsseckey_t *key, dns_rdata_t *keyrdata, const char *keystr,
	...
	if (exists(cds, &cdsrdata)) {
		...
		isc_log_write(dns_lctx, DNS_LOGCATEGORY_GENERAL,
			      ...
			      keystr);
		return delrdata(&cdsrdata, diff, origin, cds->ttl, mctx);
	}
	return DNS_R_UNCHANGED;
}
