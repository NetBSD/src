...
input_key_mouse(struct window_pane *wp, struct mouse_event *m)
{
	...
	if (m->sgr_type != ' ' && (s->mode & MODE_MOUSE_SGR)) {
		...
		len += input_split2(m->b + 32, (u_char *)&buf[len]);
		len += input_split2(x + 33, (u_char *)&buf[len]);
		len += input_split2(y + 33, (u_char *)&buf[len]);
	} else {
		if (m->b > 223)
			return;
		len = xsnprintf((char *)buf, sizeof buf, "\033[M");
	...
	log_debug("writing mouse %.*s", (int)len, (char *)buf);
	bufferevent_write(wp->event, (char *)buf, len);
}
