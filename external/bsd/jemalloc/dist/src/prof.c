...
prof_malloc_sample_object(
	...
	/* Sample hook. */
	prof_sample_hook_t local_prof_sample_hook = prof_sample_hook_get();
	if (local_prof_sample_hook != NULL) {
		prof_bt_t *bt = &tctx->gctx->bt;
		pre_reentrancy(tsd, NULL);
		local_prof_sample_hook(ptr, size, bt->vec, bt->len, usize);
		post_reentrancy(tsd);
	}
}
...
prof_free_sampled_object(
	...
	/* Unsample hook. */
	prof_sample_free_hook_t local_prof_sample_free_hook =
	    prof_sample_free_hook_get();
	if (local_prof_sample_free_hook != NULL) {
		pre_reentrancy(tsd, NULL);
		local_prof_sample_free_hook(ptr, usize);
		post_reentrancy(tsd);
	}
