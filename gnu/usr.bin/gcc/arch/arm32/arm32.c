     ...
     HOST_WIDE_INT i;
{
  ...
  if( (unsigned HOST_WIDE_INT ) i > 0xffffffff)
      return FALSE;
     ...
     enum rtx_code code, outer_code;
{
  ...
  switch (code)
    ...
    case PLUS: 
 	...
 	{
 	  unsigned HOST_WIDE_INT i = INTVAL (XEXP (x, 1)) & (unsigned
   ...
   if (code0 == REG)
       ...
       if (code1 == REG)
 	{
 	  ...
 	  otherops[1] = gen_rtx (REG, SImode, 1 + reg1);
          ...
          /* we are about to copy reg0 = reg1     operands
          *                       reg0+1 = reg1+1 otherops
          * reverse order if reg1+1 = reg0
          */
 	...
 	{
            // Bug note  nr -
            // the int may be as big as a host wide int
            // which may not fit into a single arm word
#if HOST_BITS_PER_WIDE_INT == 64            
                
                unsigned HOST_WIDE_INT v = INTVAL(operands[1]);
                unsigned HOST_WIDE_INT top = v >> BITS_PER_WORD;
                unsigned HOST_WIDE_INT bot = v & 0xffffffff;
                otherops[1] = GEN_INT(top);
                operands[1] = GEN_INT(bot);
                output_mov_immediate (operands);
                output_mov_immediate (otherops);
#else                
...
{								
  ...
  /* this assumes the whole app is compiled the same way?*/
  if (flag_pic && ! TARGET_APCS)
    arm_pic_register = 10;
}
     ...
     rtx reg;
{
  ...
  if ( ( CONSTANT_POOL_ADDRESS_P( orig ))
      ...
      {
          rtx temp = legitimize_pic_address (XEXP(orig,0), mode, reg);
          XEXP(orig,0) = temp;
      }
...
/* --- this assumes that the dest is a  s reg
-- if dest is a d reg we might be able to do this in two moves */
...
* a fixed instruction has a constant that needs to be in a const pool */
...
/*
if(mode == DFmode && (GET_CODE(x) == CONST_DOUBLE))
        return GENERAL_REGS;
*/
