...
#define LM_ADDR(so) ((so) -> smp.som_addr)
#define LM_NEXT(so) ((so) -> smp.som_next)
#define LM_NAME(so) ((so) -> smp.som_path)
static struct _dynamic dynamic_copy;
static struct section_dispatch_table sdt_copy;
...
struct so_list {
  ...
  struct so_map smp;			/* copy of link map from inferior */
  struct so_map *soaddr;		/* addr in inferior lm was read from */
...
first_so_map_member PARAMS ((void));
    ...
    struct objfile *objfile;
{
  ...
  while (rtc_symp)
	...
	{
#endif
	  len = inferior_rtc_nzlist.nz_size;
...
LOCAL FUNCTION
	...
	first_so_map_member -- locate first member in dynamic linker's map

SYNOPSIS

	static struct link_map *first_so_map_member (void)
  ...
  if (dynamic_copy.d_version >= 2)
    {
	 ...
	 structure and find the address of the link map list from it. */
      read_memory ((CORE_ADDR) dynamic_copy.d_un.d_sdt, (char *) &sdt_copy,
		   sizeof (sdt_copy));
      smp = sdt_copy.sdt_loaded;
    }
     ...
     struct so_list *so_list_ptr;	/* Last lm or NULL for first one */
{
  ...
  if (so_list_ptr == NULL)
  ...
  else
    {
	 ...
	 the shared library list.  Advance to the next shared object. */
      if ((smp = LM_NEXT (so_list_ptr)) == NULL)
	{
	  /* We have hit the end of the list, so check to see if any were
	     added, but be quiet if we can't read from the target any more. */
	  int status = target_read_memory ((CORE_ADDR) so_list_ptr -> soaddr,
					   (char *) &(so_list_ptr -> smp),
					   sizeof (struct so_map));
	  if (status == 0)
	    {
	      smp = LM_NEXT (so_list_ptr);
	    }
	  else
	    {
	      smp = NULL;
  ...
  if ((so_list_next == NULL) && (smp != NULL))
      ...
      else
	{
      ...
      so_list_next = new;
      read_memory ((CORE_ADDR) smp, (char *) &(new -> smp),
		   sizeof (struct so_map));
