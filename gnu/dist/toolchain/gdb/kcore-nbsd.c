...
init_kcore_ops()
{
  ...
  kcore_ops.to_longname = "Kernel memory image";
  kcore_ops.to_doc = "Use a kernel core dump file or live kernel as a target.\n\
Specify the filename of the core file or /dev/mem for the live kernel.";
  ...
  kcore_ops.to_insert_breakpoint = ignore;
  kcore_ops.to_remove_breakpoint = ignore;
  kcore_ops.to_create_inferior = find_default_create_inferior;
  kcore_ops.to_stratum = process_stratum;
  kcore_ops.to_has_all_memory = 0;
  kcore_ops.to_has_memory = 1;
  kcore_ops.to_has_stack = 1;
  kcore_ops.to_has_registers = 1;
  kcore_ops.to_has_execution = 0;
