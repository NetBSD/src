/*	$NetBSD: if_ed.c,v 1.111.4.1 1997/07/30 07:05:30 marc Exp $	*/
...
static inline int ed_ring_copy __P((struct ed_softc *, int, caddr_t, u_short));
	...
	u_short len;
{
	...
	if (!maxwait) {
	    printf("%s: pio_writemem failed to complete\n",
		   sc->sc_dev.dv_xname);
	}
}
	...
	int card, len;
{
	...
	if (sc->isa16bit) {
		while (len > 1) {
			bus_space_write_2(memt, memh, card,
			    *((u_int16_t *)from));
			from += 2;
			card += 2;
			len -= 2;
		...
		if (len == 1)
			bus_space_write_2(memt, memh, card, (u_int16_t)(*from));
	} else {
		while (len--)
	...
	int card, len;
{
	...
	if (sc->isa16bit) {
		while (len > 1) {
			*((u_int16_t *)to) = bus_space_read_2(memt, memh, card);
			to += 2;
			card += 2;
			len -= 2;
		}
	...
	bus_space_handle_t ioh;
{
	...
	if ((NIC_GET(iot, ioh, nicbase, ED_P0_CR) &
	...
	for (i=0; i<100; i++) {
		if ((NIC_GET(iot, ioh, nicbase, ED_P0_ISR) & ED_ISR_RST) ==
		    ED_ISR_RST) {
			/* ack the reset bit */
			NIC_PUT(iot, ioh, nicbase, ED_P0_ISR, ED_ISR_RST);
			break;
		}
		delay(100);
	}
