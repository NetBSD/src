/*	$NetBSD: arcmsr.c,v 1.20.8.3 2008/09/04 08:46:44 skrll Exp $ */
...
#include <sys/cdefs.h>
__KERNEL_RCSID(0, "$NetBSD: arcmsr.c,v 1.20.8.3 2008/09/04 08:46:44 skrll Exp $");
 ...
 * autoconf(9) glue.
 */
static int 	arc_match(struct device *, struct cfdata *, void *);
static void 	arc_attach(struct device *, struct device *, void *);
static int 	arc_detach(struct device *, int);
 ...
 */
#if NBIO > 0
static int 	arc_bioctl(struct device *, u_long, caddr_t);
...
static int
arc_match(struct device *parent, struct cfdata *match, void *aux)
{
...
arc_attach(struct device *parent, struct device *self, void *aux)
{
	...
	sc->sc_talking = 0;
	lockinit(&sc->sc_lock, PZERO, "arcdb", 0, 0);
	...
	/*
	 * Save the struct device * returned, because we could to attach
...
arc_bio_volops(struct arc_softc *sc, struct bioc_volops *bc)
{
	...
	case BIOC_VCREATE_VOLUME:
		 ...
		 * with the new volume.
		 */
		scsibus_sc = (struct scsibus_softc *)sc->sc_scsibus_dv;
...
arc_bio_inq(struct arc_softc *sc, struct bioc_inq *bi)
{
	...
	if (!sc->sc_maxraidset || !sc->sc_maxvolset || !sc->sc_cchans) {
		...
		error = arc_msgbuf(sc, request, 1, sysinfo, sizeof(*sysinfo));
		if (error != 0)
			goto out;
...
arc_lock(struct arc_softc *sc)
{
	...
	lockmgr(&sc->sc_lock, LK_EXCLUSIVE, 0);
	s = splbio();
...
arc_unlock(struct arc_softc *sc)
{
	...
	sc->sc_talking = 0;
	splx(s);
	lockmgr(&sc->sc_lock, LK_RELEASE, 0);
}
