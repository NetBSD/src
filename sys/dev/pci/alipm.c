/*	$NetBSD: alipm.c,v 1.16 2026/06/05 08:43:47 jdc Exp $ */
...
#include <sys/cdefs.h>
__KERNEL_RCSID(0, "$NetBSD: alipm.c,v 1.16 2026/06/05 08:43:47 jdc Exp $");
...
alipm_attach(device_t parent, device_t self, void *aux)
{
	...
	if ((reg & PCI_STATUS_CAPLIST_SUPPORT) == 0) {
		...
		sc->sc_iot = pa->pa_iot;
		if (iobase == 0) {
			aprint_error_dev(sc->sc_dev, "I/O base not set\n");
			return;
		}
