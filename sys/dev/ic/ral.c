/*	$NetBSD: ral.c,v 1.5.6.3 2005/12/11 10:28:50 christos Exp $ */
...
#include <sys/cdefs.h>
__KERNEL_RCSID(0, "$NetBSD: ral.c,v 1.5.6.3 2005/12/11 10:28:50 christos Exp $");
...
ral_encryption_intr(struct ral_softc *sc)
{
	...
	for (; sc->txq.next_encrypt != hw;) {
		...
		if (le32toh(desc->flags) & (RAL_TX_BUSY | RAL_TX_CIPHER_BUSY))
			break;
...
ral_decryption_intr(struct ral_softc *sc)
{
	...
	for (; sc->rxq.cur_decrypt != hw;) {
		...
		if (le32toh(desc->flags) & (RAL_RX_BUSY | RAL_RX_CIPHER_BUSY))
			break;
