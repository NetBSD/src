...
/*	$NetBSD: if_zyd.c,v 1.6.2.2 2007/06/21 15:09:41 itohy Exp $	*/
 ...
 */
#include <sys/cdefs.h>
__KERNEL_RCSID(0, "$NetBSD: if_zyd.c,v 1.6.2.2 2007/06/21 15:09:41 itohy Exp $");
...
Static void	zyd_abort_pipes(struct zyd_softc *);
...
Static void	zyd_rx_data(struct zyd_softc *, struct mbuf *, uint16_t);
...
Static int	zyd_ioctl(struct ifnet *, u_long, usb_ioctlarg_t);
...
Static void
zyd_abort_pipes(struct zyd_softc *sc)
{
	int i;
...
zyd_free_tx_list(struct zyd_softc *sc)
{
	...
	usb_ether_tx_list_free(sc->tx_data, ZYD_TX_LIST_CNT);
...
zyd_free_rx_list(struct zyd_softc *sc)
{
	...
	usb_ether_rx_list_free(sc->rx_data, ZYD_RX_LIST_CNT);
}
    ...
    void *odata, int olen, u_int flags)
{
	...
	if ((xfer = usbd_alloc_xfer(sc->sc_udev, sc->zyd_ep[ZYD_ENDPT_IOUT]))
	    == NULL)
		return ENOMEM;
...
Static void
zyd_rx_data(struct zyd_softc *sc, struct mbuf *m, uint16_t len)
{
	struct ieee80211com *ic = &sc->sc_ic;
	struct ifnet *ifp = &sc->sc_if;
	struct ieee80211_node *ni;
	...
	plcp = mtod(m, const struct zyd_plcphdr *);
	stat = (const struct zyd_rx_stat *)
	    (mtod(m, char *) + len - sizeof (struct zyd_rx_stat));
	...
	m_adj(m, sizeof(struct zyd_plcphdr));
...
zyd_rxeof(usbd_xfer_handle xfer, usbd_private_handle priv, usbd_status status)
{
	...
	struct zyd_softc *sc = (void *)data->ue_dev;
	struct ifnet *ifp = &sc->sc_if;
	...
	desc = (const struct zyd_rx_desc *)
	    (mtod(m, char *) + len - sizeof (struct zyd_rx_desc));

	if (UGETW(desc->tag) == ZYD_TAG_MULTIFRAME) {
		int pktoff[ZYD_MAX_RXFRAMECNT];
		uint16_t pktlen[ZYD_MAX_RXFRAMECNT];
		struct mbuf *pktmbuf[ZYD_MAX_RXFRAMECNT];
		int npkt, i, off;
		...
		off = 0;
		for (npkt = 0; npkt < ZYD_MAX_RXFRAMECNT; npkt++) {
			const uint16_t len16 = UGETW(desc->len[npkt]);

			if (len16 == 0 || off + len16 > len)
				break;
			pktoff[npkt] = off;
			pktlen[npkt] = len16;
		...
		for (i = 0; i < npkt; i++) {
			...
			zyd_rx_data(sc, pktmbuf[i], pktlen[i]);
		}
	} else {
		DPRINTFN(3, ("received single-frame transfer\n"));

		zyd_rx_data(sc, m, len);
	}
...
zyd_tx_mgt(struct zyd_softc *sc, struct mbuf *m0, struct ieee80211_node *ni)
{
	...
	/* save local copy against possible mbuf move */
	USB_KASSERT(sizeof i_addr1 == sizeof wh->i_addr1);
	memcpy(i_addr1, wh->i_addr1, sizeof i_addr1);
	i_fc_0 = wh->i_fc[0];
	...
	M_PREPEND(m0, sizeof(struct zyd_tx_desc), M_DONTWAIT);
	if (m0 != NULL)
		m0 = m_pullup(m0, sizeof(struct zyd_tx_desc));/* just in case */
	if (m0 == NULL) {
		return ENOBUFS;
	}
	desc = mtod(m0, struct zyd_tx_desc *);

	/* fill Tx descriptor */
	USETW((uint8_t *)&desc->len, totlen);

	desc->flags = ZYD_TX_FLAG_BACKOFF;
	if (!IEEE80211_IS_MULTICAST(i_addr1)) {
	...
	if (sc->mac_rev == ZYD_ZD1211)
		pktlen += totlen;
	USETW((uint8_t *)&desc->pktlen, pktlen);

	USETW((uint8_t *)&desc->plcp_length, (16 * totlen + rate - 1) / rate);
	desc->plcp_service = 0;
	if (rate == 22) {
	...
	usbd_setup_xfer(data->ue_xfer, sc->zyd_ep[ZYD_ENDPT_BOUT], data,
	    NULL /* XXX buf */, xferlen, USBD_FORCE_SHORT_XFER | USBD_NO_COPY,
...
zyd_txeof(usbd_xfer_handle xfer, usbd_private_handle priv, usbd_status status)
{
	...
	((struct zyd_node *)sc->tx_ni[data->ue_idx])->amn.amn_txcnt++;
...
Static int
zyd_ioctl(struct ifnet *ifp, u_long cmd, usb_ioctlarg_t data)
{
	struct zyd_softc *sc = ifp->if_softc;
	struct ieee80211com *ic = &sc->sc_ic;
	int s, error = 0;
...
zyd_init(struct ifnet *ifp)
{
	 ...
	 */
	sc->tx_queued = 0;
	if ((error = usb_ether_tx_list_init(USBDEV(sc->sc_dev),
	    sc->tx_data, ZYD_TX_LIST_CNT,
	    sc->sc_udev, sc->zyd_ep[ZYD_ENDPT_BOUT], NULL)) != 0) {
		printf("%s: could not allocate Tx list\n",
		    USBDEVNAME(sc->sc_dev));
		goto fail;
	}
	if ((error = usb_ether_rx_list_init(USBDEV(sc->sc_dev),
	    sc->rx_data, ZYD_RX_LIST_CNT,
	    sc->sc_udev, sc->zyd_ep[ZYD_ENDPT_BIN])) != 0) {
	...
	for (i = 0; i < ZYD_RX_LIST_CNT; i++) {
		struct ue_chain *data = &sc->rx_data[i];

		(void)usbd_map_buffer_mbuf(data->ue_xfer, data->ue_mbuf);
		usbd_setup_xfer(data->ue_xfer, sc->zyd_ep[ZYD_ENDPT_BIN], data,
