...
/*	$NetBSD: if_zyd.c,v 1.17.2.2 2009/03/24 21:25:04 bouyer Exp $	*/
 ...
 */
#include <sys/cdefs.h>
__KERNEL_RCSID(0, "$NetBSD: if_zyd.c,v 1.17.2.2 2009/03/24 21:25:04 bouyer Exp $");
...
USB_ATTACH(zyd)
{
	...
	if (UGETW(ddesc->bcdDevice) < 0x4330) {
		printf("%s: device version mismatch: 0x%x (only >= 43.30 supported)\n",
		    USBDEVNAME(sc->sc_dev), UGETW(ddesc->bcdDevice));
