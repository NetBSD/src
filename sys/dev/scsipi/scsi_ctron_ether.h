/*	$NetBSD: scsi_ctron_ether.h,v 1.7 2005/01/31 23:06:41 reinoud Exp $	*/
...
struct scsi_ctron_ether_recv {
	uint8_t opcode;	/* This really *is* all */
};
...
/* standard; */
	uint8_t device;		/* 3 (T_CPU) */
	uint8_t dev_qual2;		/* 0 (fixed) */
	uint8_t version;		/* 0 */
	uint8_t response_format;		/* 0 */
	uint8_t additional_len;	/* 75!! */
	uint8_t unused[2];		/* 0, 0 */
	uint8_t flags; 		/* 0x18 (sync+linked!?) */
...
/* non-standard; */
	uint8_t hwaddr[6];		/* PROM ethernet addr */
	uint8_t swaddr[6];		/* curr ethernet addr */
	char date[22];			/* firmware date string (asciz) */
	uint8_t mtype; 		/* media type?? */
	uint8_t hwport;		/* value of h/w read port?? */
};
