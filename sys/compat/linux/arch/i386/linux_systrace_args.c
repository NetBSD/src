/* $NetBSD: linux_systrace_args.c,v 1.8.10.1 2018/09/06 06:55:45 pgoyette Exp $ */
...
systrace_args(register_t sysnum, const void *params, uintptr_t *uarg, size_t *n_args)
{
	...
	/* linux_sys_break */
	case 17: {
		const struct linux_sys_break_args *p = params;
	...
	case 173: {
		const struct linux_sys_rt_sigreturn_args *p = params;
...
#define linux_sys_mmap2_args linux_sys_mmap_args
	...
	case 243: {
		const struct linux_sys_set_thread_area_args *p = params;
	...
	case 244: {
		const struct linux_sys_get_thread_area_args *p = params;
		uarg[0] = (intptr_t) SCARG(p, desc); /* struct linux_user_desc * */
		*n_args = 1;
		break;
	}
