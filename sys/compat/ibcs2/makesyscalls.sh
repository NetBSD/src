#! /bin/sh -
#	$NetBSD: makesyscalls.sh,v 1.1.2.2 1994/08/24 19:14:28 mycroft Exp $
#
...
awk < $1 "
	...
	NR == 1 {
		...
		printf "struct sysent ibcs2_sysent[] = {\n" > sysent

		printf " * created from: NetBSD %s %s %s %s\n */\n\n",$2,$3,$4,$5 > sysnames
		printf "char *ibcs2_syscallnames[] = {\n" > sysnames
	...
	$2 == "LIBCOMPAT" {
		...
		printf("\t\"old.%s\",\t\t/* %d = old %s */\n", \
		    $5, syscall, $5) > sysnames
		printf("#define\tIBCS2_SYS_%s\t%d\t/* compatibility; still used by libc */\n", \
		    $5, syscall) > syshdr
		syscall++
		next
	}
	...
	END {
		...
		printf("};\n\n") > sysent
		printf("int\tnibcs2_sysent = sizeof(ibcs2_sysent) / sizeof(ibcs2_sysent[0]);\n") > sysent
