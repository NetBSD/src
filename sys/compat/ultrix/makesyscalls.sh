...
# $Id: makesyscalls.sh,v 1.1 1994/06/02 06:25:37 glass Exp $
...
awk < $1 "
	...
	$2 == "LIBCOMPAT" {
		...
		printf("\t\"old.%s\",\t\t/* %d = old %s */\n", \
		    $5, syscall, $5) > sysnames
		printf("#define\tULTRIX_SYS_%s\t%d\t/* compatibility; still used by libc */\n", \
		    $5, syscall) > syshdr
		syscall++
		next
	}
	...
	END {
		...
		printf("};\n\n") > sysent
		printf("int\tnultrix_sysent = sizeof(ultrix_sysent) / sizeof(ultrix_sysent[0]);\n") > sysent
