/* $NetBSD: rump_syscalls.c,v 1.59.2.1 2011/06/06 09:10:07 jruoho Exp $ */
...
#include <sys/cdefs.h>
__KERNEL_RCSID(0, "$NetBSD: rump_syscalls.c,v 1.59.2.1 2011/06/06 09:10:07 jruoho Exp $");
...
rump___sysimpl_quotactl50(const char * path, struct plistref * pref)
{
	...
	error = rsys_syscall(SYS___quotactl50, &callarg, sizeof(callarg), retval);
	rsys_seterrno(error);
	if (error == 0) {
		if (sizeof(int) > sizeof(register_t))
			rv = *(int *)retval;
