/* $NetBSD: rump_syscalls.c,v 1.61.2.1 2011/01/20 14:24:59 bouyer Exp $ */
...
#include <sys/cdefs.h>
__KERNEL_RCSID(0, "$NetBSD: rump_syscalls.c,v 1.61.2.1 2011/01/20 14:24:59 bouyer Exp $");
...
int rump___sysimpl_quotactl50(const char *, struct plistref *);
int
rump___sysimpl_quotactl50(const char * path, struct plistref * pref)
{
	register_t rval[2] = {0, 0};
	int error = 0;
	struct sys___quotactl50_args callarg;

	SPARG(&callarg, path) = path;
	SPARG(&callarg, pref) = pref;

	error = rsys_syscall(SYS___quotactl50, &callarg, sizeof(callarg), rval);
	if (error) {
		rval[0] = -1;
		rsys_seterrno(error);
	}
...
rump_sys_pipe(int *fd)
{
	...
	error = rsys_syscall(SYS_pipe, NULL, 0, rval);
	if (error) {
		rsys_seterrno(error);
	} else {
		fd[0] = rval[0];
