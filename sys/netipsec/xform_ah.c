/*	$NetBSD: xform_ah.c,v 1.1 2003/08/13 20:06:51 jonathan Exp $	*/
...
#include <sys/cdefs.h>
__KERNEL_RCSID(0, "$NetBSD: xform_ah.c,v 1.1 2003/08/13 20:06:51 jonathan Exp $");
...
ah_input(struct mbuf *m, struct secasvar *sav, int skip, int protoff)
{
	...
	DPRINTF(("ah_input skip %d poff %d\n"
		 "len: hl %d authsize %d rpl %d expect %d\n",
		 skip, protoff,
		 hl, authsize, rplen,
		 authsize + rplen - sizeof(struct ah)));
