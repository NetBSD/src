/* $NetBSD: cfb.c,v 1.1.2.1 1998/10/15 02:48:59 nisimura Exp $ */

#include <sys/cdefs.h>			/* RCS ID & Copyright macro defns */

__KERNEL_RCSID(0, "$NetBSD: cfb.c,v 1.1.2.1 1998/10/15 02:48:59 nisimura Exp $");
	...
	void *arg;
{
	...
	*(u_int8_t *)(cfbbase + CFB_OFFSET_IREQ) = 0;
	...
	if (v & DATA_CURCMAP_CHANGED) {
		...
		vdac->bt_cmap = cp[1];	wbflush();
		vdac->bt_cmap = cp[3];	wbflush();
		vdac->bt_cmap = cp[5];	wbflush();

		BT459_SELECT(vdac, BT459_REG_CCOLOR_3);
		vdac->bt_cmap = cp[0];	wbflush();
		vdac->bt_cmap = cp[2];	wbflush();
		vdac->bt_cmap = cp[4];	wbflush();
	}
