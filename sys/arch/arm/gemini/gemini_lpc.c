/*	$NetBSD: gemini_lpc.c,v 1.5.8.1 2021/08/04 16:51:27 thorpej Exp $	*/

#include "opt_gemini.h"
#include "locators.h"

#include <sys/cdefs.h>
__KERNEL_RCSID(0, "$NetBSD: gemini_lpc.c,v 1.5.8.1 2021/08/04 16:51:27 thorpej Exp $");
...
gemini_lpc_attach(device_t parent, device_t self, void *aux)
{
	...
	config_search(self, NULL,
	    CFARGS(.search = gemini_lpc_search));
}
...
gemini_lpc_search(device_t parent, cfdata_t cf, const int *ldesc, void *aux)
{
	...
	if (config_probe(parent, cf, &lpc)) {
		config_attach(parent, cf, &lpc, gemini_lpc_busprint, CFARGS_NONE);
		return 0;			/* love it */
	}
