/*	$NetBSD: ofw.c,v 1.1.2.3 2002/02/11 20:07:40 jdolecek Exp $	*/
...
struct vl_isa_range {
	vaddr_t isa_phys_hi;
	vaddr_t isa_phys_lo;
	vaddr_t parent_phys_hi;
	vaddr_t parent_phys_lo;
	vm_size_t   isa_size;
};
...
static void ofw_claimpages __P((vaddr_t *, pv_addr_t *, vm_size_t));
static void ofw_discardmappings __P ((vaddr_t, vaddr_t, vm_size_t));
static int ofw_mem_ihandle  __P((void));
static int ofw_mmu_ihandle  __P((void));
static vaddr_t ofw_claimphys __P((vaddr_t, vm_size_t, vaddr_t));
#if 0
static vaddr_t ofw_releasephys __P((vaddr_t, vm_size_t));
#endif
static vaddr_t ofw_claimvirt __P((vaddr_t, vm_size_t, vaddr_t));
static void ofw_settranslation __P ((vaddr_t, vaddr_t, vm_size_t, int));
static void ofw_initallocator __P((void));
static void ofw_configisaonly __P((vaddr_t *, vaddr_t *));
static void ofw_configvl __P((int, vaddr_t *, vaddr_t *));
static vaddr_t ofw_valloc __P((vaddr_t, vaddr_t));
