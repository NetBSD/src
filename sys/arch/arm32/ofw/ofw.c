    ...
    struct ofw_cbargs *args;
{
	...
	/*
	 *  Note that we are running in the IRQ frame, with interrupts disabled.
	 *  
	 ...
	 *      Our handler will resume when it finishes with the code that
	...
	/* Sanity checks. */
	/* For now, I am only willing to translate va's in the "ofw range." */
	/* Eventually, I may be more generous. -JJK */
	if ((va & PGOFSET) != 0 ||
	    va < OFW_VIRT_BASE ||
    ...
    vm_offset_t *proc0_ptpt;
{
	...
	ofw_settranslation(systempage.virtual, systempage.physical, NBPG, -1);	/* XXX - mode? -JJK */
	...
	for (oft = 0, 
		...
		/* The only allowable regions are page0, the kernel-static */
		/* area, and the ofw area. */
		switch(va >> (PDSHIFT + 2)) {
		    case 0:
			/* page0 */
			...
			/* ofw area */
			L2pagetable = proc0_pt_ofw[(va >> (PDSHIFT + 2)) & 0x3].virtual;
			break;
			...
			/* io area */
			L2pagetable = proc0_pt_io[(va >> (PDSHIFT + 2)) & 0x3].virtual;
			break;
	...
	for (i = 0; i < 8; i++)
	    map_entry_nc(L2pagetable, ((KERNEL_VM_BASE + i * 0x00400000) >> (PGSHIFT-2)),
			 proc0_pt_vmdata[i].physical);
	for (i = 0; i < 4; i++)
	    map_entry_nc(L2pagetable, ((0xf5000000 + i * 0x00400000) >> (PGSHIFT-2)),
			 proc0_pagedir.physical + i * NBPG);
	for (i = 0; i < 4; i++)
	    map_entry_nc(L2pagetable, ((OFW_VIRT_BASE + i * 0x00400000) >> (PGSHIFT-2)),
			 proc0_pt_ofw[i].physical);
	for (i = 0; i < 4; i++)
	    map_entry_nc(L2pagetable, ((IO_VIRT_BASE + i * 0x00400000) >> (PGSHIFT-2)),
			 proc0_pt_io[i].physical);
    }

    /* Construct the proc0 L1 pagetable. */
    {
	vm_offset_t L1pagetable = proc0_pagedir.virtual;
	struct mem_translation *tp;

	map_pagetable(L1pagetable, 0x0,
		      proc0_pt_sys.physical);
	...
	for (oft = 0, 
	  ...
	  if (((va & (NBPD - 1)) == 0) && ((pa & (NBPD - 1)) == 0)) {
	    ...
	    while (nsections--) {
	      ...
	      if ((tp->mode & 0xC) == 0xC)
		((u_int *)L1pagetable)[(va >> PDSHIFT)] |= PT_B | PT_C;
...
ofw_getphysmeminfo()
{
    ...
    if ((phandle = OF_finddevice("/memory")) == -1 ||
	 ...
	 OF_getprop(phandle, "available", OFphysavail, avail_len) != avail_len)
	...
	for (j = 0, mp = tmp; j < cnt; j++, mp++) {
	    ...
	    /* Handle empty block. */
	    if (mp->size == 0) {
		bcopy(mp + 1, mp, (cnt - (mp - tmp)) * sizeof(struct mem_region));
		cnt--;
		mp--;
		continue;
	    }
...
static vm_offset_t
ofw_valloc(vm_offset_t size, vm_offset_t align)
{
  PVFREE        *ppvf;
  PVFREE        pNew;
  vm_offset_t   new;
  ...
  for (ppvf = &vflist; *ppvf; ppvf = &((*ppvf)->pNext)) {
    ...
    if (((*ppvf)->size - lead) >= size) {
      if (lead == 0) {
	if (size == (*ppvf)->size) { /* using whole block */
	  (*ppvf) = (*ppvf)->pNext; /* splice out of list */
	} else { /* tail of block is free */
	  (*ppvf)->start = new + size;
	  (*ppvf)->size -= size;
	}
      } else {
	vm_size_t tail = ((*ppvf)->start + (*ppvf)->size) - (new + size);
	(*ppvf)->size = lead; /* free space at beginning */
...
vm_offset_t
ofw_map(vm_offset_t pa, vm_size_t size, int cb_bits)
{
  vm_offset_t va;
...
/*printf("ofw_gettranslation (%x) --> ", va);*/
    ...
    if (OF_call_method("translate", mmu_ihandle, 1, 3, va, &pa, &mode, &exists) != 0)
	return(-1);
...
ofw_malloc(vm_size_t size)
{
  ...
  if (*ppLeftover) { /* have a leftover of the right size */
    ...
    if ((*ppLeftover)->size < (size + sizeof(LEFTOVER))) {
      *ppLeftover = (*ppLeftover)->pNext; /* splice out of chain */
    } else {
      pLeft = (*ppLeftover)->pNext; /* remember the next pointer */
      newSize = (*ppLeftover)->size - size; /* reduce size */
      /* move pointer */
      *ppLeftover = (PLEFTOVER)(((vm_offset_t)*ppLeftover) + size); 
...
#if 0
static void
ofw_free(vm_offset_t addr, vm_size_t size)
{
...
ofw_claimpages(free_pp, pv_p, size)
    ...
    vm_size_t size;
{
    vm_size_t alloc_size = (size + NBPG	- 1) & ~(NBPG -	1);	/* round-up to page boundary */
    ...
    ofw_settranslation(va, pa, alloc_size, -1);	    /* XXX - what mode? -JJK */
